...
import { IntangibleAssetsScheduleData, ManualInput } from '../../../types.ts';

interface IntangibleAssetsNoteProps {
...
// Reusing the structure of PPENote but with "Amortisation" as labels
export const IntangibleAssetsNote: React.FC<IntangibleAssetsNoteProps> = ({ data }) => {
    ...
    return (
        <div className="space-y-4">
            ...
            {shouldShow(researchAndDevelopmentExpense) && (
                <div className="text-xs mt-2">
                    <p className="font-semibold text-gray-400">R&D Expenditure Recognised as Expense:</p>
                    <p className="italic text-gray-500">{getValue(researchAndDevelopmentExpense)}</p>
                </div>
            )}
        </div>
    );
