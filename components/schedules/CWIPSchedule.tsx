...
import { CWIPRow, ScheduleData } from '../../types.ts';
import { PlusIcon, TrashIcon } from '../icons.tsx';

interface CWIPScheduleProps {
    data: CWIPRow[];
    onUpdate: React.Dispatch<React.SetStateAction<ScheduleData>>;
    isFinalized: boolean;
}

export const CWIPSchedule: React.FC<CWIPScheduleProps> = ({ data, onUpdate, isFinalized }) => {

    const handleUpdate = (id: string, field: keyof Omit<CWIPRow, 'id'>, value: string) => {
        onUpdate(prev => ({ ...prev, cwip: prev.cwip.map(row => row.id === id ? { ...row, [field]: value } : row) }));
    };

    const addRow = () => {
        const newRow: CWIPRow = { id: uuidv4(), particular: '', opening: '', additions: '', capitalized: '', closing: '' };
        onUpdate(prev => ({ ...prev, cwip: [...prev.cwip, newRow] }));
    };

    const removeRow = (id: string) => {
        onUpdate(prev => ({ ...prev, cwip: prev.cwip.filter(row => row.id !== id) }));
    };

    const renderCell = (row: CWIPRow, field: keyof Omit<CWIPRow, 'id' | 'closing'>) => (
        <td className="p-0">
            <input
                type="text"
                value={row[field]}
    ...
    return (
        <div className="space-y-4">
            <h3 className="text-lg font-semibold text-white">Capital Work-in-Progress (CWIP)</h3>
                                ...
                                <tr key={row.id} className="hover:bg-gray-700/30">
                                    <td className="p-0 flex items-center">
                                        ...
                                        <input
                                            ...
                                            onChange={e => handleUpdate(row.id, 'particular', e.target.value)}
                                            disabled={isFinalized}
