...
interface ChangesInInventoriesScheduleProps {
        ...
        <div className="space-y-2">
            {rows.map(row => (
                <div key={row.id} className="flex items-center space-x-2">
                    <input type="text" placeholder="Inventory Name" value={row.name} onChange={e => onUpdate(type, row.id, 'name', e.target.value)} disabled={isFinalized} className="w-1/2 bg-gray-700 p-2 rounded-md"/>
...
export const ChangesInInventoriesSchedule: React.FC<ChangesInInventoriesScheduleProps> = ({ data, onUpdate, isFinalized }) => {

    const handleUpdate = (type: 'opening' | 'closing', id: string, field: 'name' | 'amountCy' | 'amountPy', value: string) => {
        onUpdate(prev => ({ ...prev, changesInInventories: { ...prev.changesInInventories, [type]: prev.changesInInventories[type].map(r => r.id === id ? { ...r, [field]: value } : r) }}));
    };

    const addRow = (type: 'opening' | 'closing') => {
        const newRow: InventoryRow = { id: uuidv4(), name: '', amountCy: '', amountPy: '' };
        onUpdate(prev => ({ ...prev, changesInInventories: { ...prev.changesInInventories, [type]: [...prev.changesInInventories[type], newRow] }}));
    };

    const removeRow = (type: 'opening' | 'closing', id: string) => {
        onUpdate(prev => ({ ...prev, changesInInventories: { ...prev.changesInInventories, [type]: prev.changesInInventories[type].filter(r => r.id !== id) }}));
    };
    
    return (
        <div className="space-y-6">
            <h3 className="text-lg font-semibold text-white">Changes in Inventories Schedule</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <InventoryTable title="Opening Inventories" rows={data.opening} onUpdate={handleUpdate} onAdd={addRow} onRemove={removeRow} type="opening" isFinalized={isFinalized}/>
                <InventoryTable title="Closing Inventories" rows={data.closing} onUpdate={handleUpdate} onAdd={addRow} onRemove={removeRow} type="closing" isFinalized={isFinalized}/>
            </div>
        </div>
    );
};