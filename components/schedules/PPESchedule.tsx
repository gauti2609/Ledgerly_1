...
export const PPESchedule: React.FC<PPEScheduleProps> = ({ data, onUpdate, isFinalized }) => {
    ...
    const handleFieldUpdate = (field: 'commitments' | 'pledgedAssets' | 'borrowingCostsCapitalized', value: ManualInput) => {
    ...
    return (
        <div className="space-y-4">
                                ...
                                <tr key={row.id} className="hover:bg-gray-700/30">
                                    <td className="p-0 border border-gray-600">
                                        <div className="flex items-center">
                                            ...
                                            <input type="text" value={row.assetClass} onChange={e => handleAssetUpdate(row.id, 'assetClass', e.target.value)} disabled={isFinalized} className="flex-1 bg-transparent p-2 border-none focus:ring-0 focus:outline-none focus:bg-gray-700/50" placeholder="e.g., Buildings" />
            ...
            <div className="mt-6 space-y-4">
                <InputWithCheckbox
                    label="Contractual commitments for the acquisition of PPE"
                    value={commitments}
                ...
                />
                <InputWithCheckbox
                    label="Assets pledged as security"
                    value={pledgedAssets}
                ...
                />
                <InputWithCheckbox
                    label="Borrowing Costs Capitalized (AS 16)"
                    value={borrowingCostsCapitalized}
                    onChange={v => handleFieldUpdate('borrowingCostsCapitalized', v)}
                    disabled={isFinalized}
                    rows={2}
                />
            </div>
