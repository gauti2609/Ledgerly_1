...
// Default masters with Standard groupings (Schedule III compliant)
...
export class MastersService {
    ...
    /**
     * Sync masters to the updated Standard groupings structure for all entities belonging to a user.
     * This replaces the masters data with the current DEFAULT_MASTERS.
     */
    async syncMastersForAllEntities(userId: string): Promise<{ entitiesUpdated: number }> {
        ...
        for (const entity of entities) {
            ...
            // Replace masters with the updated structure
            data.masters = DEFAULT_MASTERS;
        ...
        console.log(`[MastersService] Synced masters (Standard groupings) for ${updated} entities`);
        return { entitiesUpdated: updated };
    }

    /**
     * Add a new grouping to all entities belonging to a user.
     * This ensures the grouping is available across all entities (global sync).
     */
    async addGroupingToAllEntities(userId: string, grouping: Grouping): Promise<void> {
        // Get all entities for this user
        const entities = await this.prisma.financialEntity.findMany({
            where: { userId },
        });

        // Update each entity's masters with the new grouping
        for (const entity of entities) {
            const data = entity.data as any;

            // Initialize masters structure if not present
            if (!data.masters) {
                data.masters = DEFAULT_MASTERS;
            }

            // Check if grouping already exists (by code)
            const existingGrouping = data.masters.groupings?.find(
                (g: Grouping) => g.code === grouping.code,
            );

            if (!existingGrouping) {
                // Add the new grouping
                data.masters.groupings = data.masters.groupings || [];
                data.masters.groupings.push(grouping);
        ...
        console.log(
            `[MastersService] Added grouping '${grouping.code}' to ${entities.length} entities`,
        );
    }

    /**
     * Get the default masters data (Standard groupings).
     */
    getDefaultMasters(): Masters {
